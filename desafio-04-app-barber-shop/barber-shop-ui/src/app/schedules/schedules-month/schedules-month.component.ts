...
export class SchedulesMonthComponent implements OnInit, OnDestroy {
  ...
  // Inicializa monthSchedule com valores default para evitar undefined
  ...
  private fetchSchedules(currentDate: Date) {
    ...
    // Atenção: Certifique-se de que o método listInMonth constrói uma URL válida, incluindo o protocolo (http:// ou https://).
    this.subscriptions.push(
      this.httpService.listInMonth(year, month).subscribe((data: ScheduleAppointementMonthModel) => this.monthSchedule = data)
    );
  }
}